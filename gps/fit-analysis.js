...
export function analyzeSamples(samples,t0,options={}){
  ...
  const durationS=samples.at(-1).tSec,moving=samples.filter(s=>s.speedKmh>2.16),movingTimeS=moving.reduce((a,s)=>a+s.dt,0),distanceM=samples.reduce((a,s)=>a+s.dInc,0),smoothed=smooth(samples.map(s=>s.speedKmh),3),smoothedTopSpeedKmh=Math.max(...smoothed),smoothedTopSpeedIndex=smoothed.reduce((best,v,i)=>v>smoothed[best]?i:best,0),rawTopSpeedKmh=Math.max(...samples.map(s=>s.speedKmh)),topSpeedKmh=smoothedTopSpeedKmh,topSpeedSample=samples[smoothedTopSpeedIndex]||null,avgSpeedKmh=moving.length?mean(moving.map(s=>s.speedKmh)):0;
