...
export function analyzeSamples(samples,t0,options={}){
  ...
  const durationS=samples.at(-1).tSec,moving=samples.filter(s=>s.speedKmh>2.16),movingTimeS=moving.reduce((a,s)=>a+s.dt,0),calculatedDistance=samples.reduce((a,s)=>a+s.dInc,0),hasEngineDistance=engine?.metrics?.distanceMeters!=null&&Number.isFinite(Number(engine.metrics.distanceMeters)),distanceM=hasEngineDistance?Number(engine.metrics.distanceMeters):calculatedDistance,smoothed=smooth(samples.map(s=>s.speedKmh),3),smoothedTopSpeedKmh=Math.max(...smoothed),smoothedTopSpeedIndex=smoothed.reduce((best,v,i)=>v>smoothed[best]?i:best,0),rawTopSpeedKmh=Math.max(...samples.map(s=>s.speedKmh)),hasEngineTop=engine?.metrics?.maxSpeed3sMps!=null&&Number.isFinite(Number(engine.metrics.maxSpeed3sMps)),engineTop=hasEngineTop?Number(engine.metrics.maxSpeed3sMps)*3.6:null,topSpeedKmh=hasEngineTop?engineTop:smoothedTopSpeedKmh,hasPeakTime=engine?.metrics?.peak3sAtSec!=null&&Number.isFinite(Number(engine.metrics.peak3sAtSec)),topSpeedSample=hasPeakTime?samples.reduce((best,s)=>Math.abs(s.tSec-engine.metrics.peak3sAtSec)<Math.abs(best.tSec-engine.metrics.peak3sAtSec)?s:best,samples[0]):samples[smoothedTopSpeedIndex]||null,avgSpeedKmh=moving.length?mean(moving.map(s=>s.speedKmh)):0;
