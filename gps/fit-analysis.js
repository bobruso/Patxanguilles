import{buildFieldTransform,chooseNearestPitch,normalizeAttackU,periodIndexAt}from'./field-transform.js';
import{detectFootballPeriods}from'./periods.js';
...
function buildPosition(samples,{attackDirection=1,field=null,periods=[],firstPeriodAttack=null}={}){
  ...
  const fieldCalibrated=!!fieldTf,periodList=Array.isArray(periods)?periods:[],first=Number(firstPeriodAttack),hasPeriods=periodList.length>0,canOrient=fieldCalibrated&&(first===1||first===-1)&&hasPeriods;
  ...
  for(const s of gps){
    ...
    const periodIdx=hasPeriods?periodIndexAt(s.tSec,periodList):-1;if(hasPeriods&&periodIdx<0)continue;
    if(canOrient){const oriented=normalizeAttackU(u,s.tSec,{periods:periodList,firstPeriodAttack:first});if(oriented.u==null)continue;u=oriented.u}else{u=clamp(u,0,1);if(!fieldCalibrated&&Number(attackDirection)<0)u=1-u}
    v=clamp(v,0,1);const weight=s.dt||.5;totalTime+=weight;
  ...
  return{grid,zoneGrid,avgPosition,trail,points:pts,thirds:thirds.map(v=>v/(totalTime||1)),sides:sides.map(v=>v/(totalTime||1)),orientationReliable:canOrient,fieldCalibrated,lengthM:fieldTf?.lengthM||null,widthM:fieldTf?.widthM||null};
}
...
function roleEstimate(pos,ctx){
  ...
  const notes=[];if(att>.45)notes.push(pos.orientationReliable?'Gran parte del tiempo aparece en campo rival.':'Gran parte del tiempo aparece en uno de los extremos longitudinales del campo.');else if(def>.45)notes.push(pos.orientationReliable?'Tendencia clara a ocupar zonas defensivas.':'Tendencia clara a ocupar uno de los extremos longitudinales del campo.');else notes.push('Ocupación bastante repartida alrededor del centro del campo.');if(wide>.48)notes.push('Perfil muy abierto hacia banda.');if(roam>.22)notes.push('Amplio radio de acción.');if(ctx.sprints.length>=8)notes.push('Volumen alto de sprints para fútbol 7.');if(!pos.orientationReliable)notes.push(pos.fieldCalibrated?'Campo calibrado, pero falta definir periodos y dirección de ataque.':'Posición provisional: mejorará al calibrar las cuatro esquinas del campo.');
...
function fatigue(samples,highIntensityKmh,periods=[]){if(!samples.length)return null;const calc=a=>{const timeS=a.reduce((s,x)=>s+(x.dt||0),0),distanceM=a.reduce((s,x)=>s+(x.dInc||0),0),hiDistanceM=a.filter(x=>x.speedKmh>=highIntensityKmh).reduce((s,x)=>s+(x.dInc||0),0);return{timeS:+timeS.toFixed(1),distanceM:+distanceM.toFixed(1),hiDistanceM:+hiDistanceM.toFixed(1),distancePerMin:timeS?+(distanceM/(timeS/60)).toFixed(1):0}};let firstSamples,secondSamples,provisional=true;if(Array.isArray(periods)&&periods.length>=2){const a=periods[0],b=periods[1];firstSamples=samples.filter(s=>s.tSec>=Number(a.startSec)&&s.tSec<=Number(a.endSec));secondSamples=samples.filter(s=>s.tSec>=Number(b.startSec)&&s.tSec<=Number(b.endSec));provisional=false}else{const split=samples.at(-1).tSec/2;firstSamples=samples.filter(s=>s.tSec<=split);secondSamples=samples.filter(s=>s.tSec>split)}const first=calc(firstSamples),second=calc(secondSamples);return{first,second,distanceRateChangePct:first.distancePerMin?+((second.distancePerMin-first.distancePerMin)/first.distancePerMin*100).toFixed(1):0,provisional}}

export async function analyzeFit(buf,options={}){
  const sprintKmh=Number(options.sprintKmh)||DEFAULT_SPRINT_KMH,highIntensityKmh=Number(options.highIntensityKmh)||DEFAULT_HIGH_INTENSITY_KMH,attackRaw=Number(options.firstPeriodAttack??options.attackDirection),firstPeriodAttack=(attackRaw===1||attackRaw===-1)?attackRaw:null;
  ...
  const selectedPitch=options.field||chooseNearestPitch(samples,options.fields||[]),periodDetection=detectFootballPeriods(samples),periods=(Array.isArray(options.periods)&&options.periods.length)?options.periods:periodDetection.periods,sprints=detectRuns(samples,sprintKmh),hiRuns=detectRuns(samples,highIntensityKmh),highIntensityDistanceM=samples.filter(s=>s.speedKmh>=highIntensityKmh).reduce((a,s)=>a+s.dInc,0),hrs=samples.filter(s=>s.hr>0).map(s=>s.hr),pos=buildPosition(samples,{attackDirection:firstPeriodAttack||1,field:selectedPitch,periods,firstPeriodAttack}),refMax=Number(options.maxHr)||((Number(options.age)>0)?Math.round(208-.7*Number(options.age)):null)||(hrs.length?Math.max(...hrs):null),accels=accelEvents(samples),cadences=samples.filter(s=>s.cadence>0).map(s=>s.cadence);
  const role=roleEstimate(pos,{sprints,totalDistance:distanceM,durationS});
  const detail={
    positional:{thirds:pos?.thirds||[],sides:pos?.sides||[],role,orientationReliable:!!pos?.orientationReliable,fieldCalibrated:!!pos?.fieldCalibrated,pitchId:selectedPitch?.id??null,pitchName:selectedPitch?.name||null,goalALabel:selectedPitch?.goal_a_label||'Portería A',goalBLabel:selectedPitch?.goal_b_label||'Portería B',pitchDistanceFromTrackM:selectedPitch?.distanceFromTrackM??null,lengthM:pos?.lengthM??null,widthM:pos?.widthM??null,periods,periodDetection,firstPeriodAttack},
    ...
    workload:{distancePerMin:movingTimeS?+(distanceM/(movingTimeS/60)).toFixed(1):0,highIntensityDistancePerMin:movingTimeS?+(highIntensityDistanceM/(movingTimeS/60)).toFixed(1):0,fatigue:fatigue(samples,highIntensityKmh,periods),avgCadence:cadences.length?+mean(cadences).toFixed(1):null}
  };
