...
/* PATX_HR_RECOVERY_V225 · ventanas de recuperación tras transición a baja intensidad */
function detectHrRecoveryWindows(samples,referenceMaxBpm){
  const src=(samples||[]).filter(s=>Number.isFinite(Number(s.tSec))&&Number(s.hr)>0);if(src.length<8)return{events:[],summary:{count:0,avgDropBpm:null,bestDropBpm:null}};
  const events=[];let lastEnd=-9999;const minStart=Math.max(115,Number(referenceMaxBpm)>0?Number(referenceMaxBpm)*.68:115);
  for(let i=1;i<src.length-2;i++){
    const prev=src[i-1],cur=src[i],ps=Number(prev.speedKmh)||0,cs=Number(cur.speedKmh)||0;if(!(ps>=7&&cs<7))continue;if(Number(cur.tSec)<lastEnd+20)continue;
    const peakWindow=src.filter(s=>Number(s.tSec)>=Number(cur.tSec)-20&&Number(s.tSec)<=Number(cur.tSec)+10);if(!peakWindow.length)continue;
    const peak=peakWindow.reduce((a,b)=>Number(b.hr)>Number(a.hr)?b:a,peakWindow[0]);if(Number(peak.hr)<minStart)continue;let low=null;
    for(const s of src){const dt=Number(s.tSec)-Number(peak.tSec);if(dt<10||dt>75)continue;if((Number(s.speedKmh)||0)>8.5)continue;if(!low||Number(s.hr)<Number(low.hr))low=s}
    if(!low)continue;const duration=Number(low.tSec)-Number(peak.tSec),drop=Number(peak.hr)-Number(low.hr);if(duration<10||drop<12)continue;
    events.push({startSec:+Number(peak.tSec).toFixed(1),endSec:+Number(low.tSec).toFixed(1),durationS:+duration.toFixed(1),startHr:Math.round(Number(peak.hr)),endHr:Math.round(Number(low.hr)),dropBpm:Math.round(drop)});lastEnd=Number(low.tSec);
  }
  const drops=events.map(e=>e.dropBpm);return{events:events.slice(0,50),summary:{count:events.length,avgDropBpm:drops.length?+mean(drops).toFixed(1):null,bestDropBpm:drops.length?Math.max(...drops):null}};
}
...
function buildPosition(samples,{attackDirection=1,field=null}={}){
  ...
  const thirds=[0,0,0],physicalThirds=[0,0,0],sides=[0,0,0];let totalTime=0;
  for(const s of gps){
    let{u,v}=tf.project(s.lat,s.lon);if(!Number.isFinite(u)||!Number.isFinite(v))continue;
    u=clamp(u,0,1);v=clamp(v,0,1);
    const tacticalU=direction<0?1-u:u;
    const weight=s.dt||.5;totalTime+=weight;
    // IMPORTANTE v222: grid, zonas, posición media y recorrido son SIEMPRE físicos.
    ...
    physicalThirds[Math.min(2,Math.floor(u*3))]+=weight;
    // Solo la lectura táctica defensa/medio/ataque usa la orientación elegida.
    thirds[Math.min(2,Math.floor(tacticalU*3))]+=weight;
    sides[Math.min(2,Math.floor(v*3))]+=weight;
  ...
  return{grid,zoneGrid,avgPosition,trail,points:pts,thirds:thirds.map(v=>v/(totalTime||1)),physicalThirds:physicalThirds.map(v=>v/(totalTime||1)),sides:sides.map(v=>v/(totalTime||1)),attackDirection:direction,fieldCalibrated:!!fieldTf,autoBounds:fieldTf?null:(tf.autoBounds||null),lengthM:fieldTf?.lengthM||null,widthM:fieldTf?.widthM||null};
}
...
function sprintSpatialProfile(pos,sprints){
  ...
  const direction=Number(pos.attackDirection)===-1?-1:1,thirds=[0,0,0],sides=[0,0,0],points=[];
  for(const sp of sprints){
    ...
    const physicalU=clamp(Number(best.u)||0,0,.9999),tacticalU=direction<0?1-physicalU:physicalU;
    thirds[Math.min(2,Math.floor(tacticalU*3))]++;
    sides[Math.min(2,Math.floor(clamp(Number(best.v)||0,0,.9999)*3))]++;
    // El punto se conserva en coordenada física para dibujarlo siempre en el mismo sitio.
    points.push({u:+physicalU.toFixed(4),v:+Number(best.v).toFixed(4),tSec:+Number(sp.tSec).toFixed(1),peakSpeedKmh:+Number(sp.peakSpeedKmh||0).toFixed(2)});
  }
...
function roleEstimate(pos,ctx){
  if(!pos)return null;
  const sprintProfile=sprintSpatialProfile(pos,ctx.sprints),direction=Number(pos.attackDirection)===-1?-1:1;
  const tacticalAvg=pos.avgPosition?{...pos.avgPosition,u:direction<0?1-Number(pos.avgPosition.u||0):Number(pos.avgPosition.u||0)}:null;
  return roleFromFeatures({thirds:pos.thirds,sides:pos.sides,spreadU:std(pos.points.map(p=>p.u)),spreadV:std(pos.points.map(p=>p.v)),sprintCount:ctx.sprints.length,durationS:ctx.durationS,avgPosition:tacticalAvg,attackDirection:pos.attackDirection,sprintThirds:sprintProfile.thirds,sprintSides:sprintProfile.sides});
}
...
export function analyzeSamples(samples,t0,options={}){
  const highIntensityKmh=Number(options.highIntensityKmh)||DEFAULT_HIGH_INTENSITY_KMH,attackDirection=Number(options.attackDirection)===-1?-1:1,engine=options.gpsEngine||null,useGps=engine?.recording?.denseGps;
  if(useGps)for(const s of samples)if(Number.isFinite(Number(s.gpsSpeedKmh)))s.speedKmh=Number(s.gpsSpeedKmh);
  ...
  for(let i=1;i<samples.length;i++){const s=samples[i],p=samples[i-1];let d=useGps&&Number.isFinite(Number(s.dInc))?Number(s.dInc):null;if(d==null&&s.distance!=null&&p.distance!=null){d=s.distance-p.distance;if(d<0||d>60)d=null}if(d==null&&s.dt>0)d=s.speedKmh/3.6*s.dt;if(d==null&&s.lat!=null&&p.lat!=null)d=haversine(p.lat,p.lon,s.lat,s.lon);s.dInc=Math.max(0,Number.isFinite(d)?d:0)}
  const durationS=samples.at(-1).tSec,moving=samples.filter(s=>s.speedKmh>2.16),movingTimeS=moving.reduce((a,s)=>a+s.dt,0),calculatedDistance=samples.reduce((a,s)=>a+s.dInc,0),hasEngineDistance=engine?.metrics?.distanceMeters!=null&&Number.isFinite(Number(engine.metrics.distanceMeters)),distanceM=hasEngineDistance?Number(engine.metrics.distanceMeters):calculatedDistance,smoothed=smooth(samples.map(s=>s.speedKmh),3),smoothedTopSpeedKmh=Math.max(...smoothed),smoothedTopSpeedIndex=smoothed.reduce((best,v,i)=>v>smoothed[best]?i:best,0),rawTopSpeedKmh=Math.max(...samples.map(s=>s.speedKmh)),fitTopSpeedKmh=engine?.metrics?.rawFitMaxSpeedKmh!=null&&Number.isFinite(Number(engine.metrics.rawFitMaxSpeedKmh))?Number(engine.metrics.rawFitMaxSpeedKmh):null,fitTopSpeedIndex=samples.reduce((best,s,i)=>{const v=num(s.enhancedSpeedKmh??s.fitSpeedKmh),b=num(samples[best]?.enhancedSpeedKmh??samples[best]?.fitSpeedKmh);return v!=null&&(b==null||v>b)?i:best},0),fitTopSpeedSample=samples[fitTopSpeedIndex]||null,fitTopSampleKmh=fitTopSpeedSample?num(fitTopSpeedSample.enhancedSpeedKmh??fitTopSpeedSample.fitSpeedKmh):null,hasEngineTop=engine?.metrics?.maxSpeed3sMps!=null&&Number.isFinite(Number(engine.metrics.maxSpeed3sMps)),engineTop=hasEngineTop?Number(engine.metrics.maxSpeed3sMps)*3.6:null,hasPeakTime=engine?.metrics?.peak3sAtSec!=null&&Number.isFinite(Number(engine.metrics.peak3sAtSec)),fallbackTopSpeedSample=hasPeakTime?samples.reduce((best,s)=>Math.abs(s.tSec-engine.metrics.peak3sAtSec)<Math.abs(best.tSec-engine.metrics.peak3sAtSec)?s:best,samples[0]):samples[smoothedTopSpeedIndex]||null,topSpeedKmh=fitTopSpeedKmh!=null?fitTopSpeedKmh:(hasEngineTop?engineTop:smoothedTopSpeedKmh),topSpeedMethod=fitTopSpeedKmh!=null?'fit-peak':hasEngineTop?(engine?.metrics?.maxSpeedSource||'gps-3s-fallback'):'moving_average_3',topSpeedSample=fitTopSpeedKmh!=null?(fitTopSampleKmh!=null&&Math.abs(fitTopSampleKmh-fitTopSpeedKmh)<=.2?fitTopSpeedSample:null):fallbackTopSpeedSample,avgSpeedKmh=moving.length?mean(moving.map(s=>s.speedKmh)):0;
  ...
  const hrRecovery=detectHrRecoveryWindows(samples,refMax);
  const sprintProfile=sprintSpatialProfile(pos,relative.sprints),role=roleEstimate(pos,{sprints:relative.sprints,durationS});
  const detail={gpsEngine:engine?{version:engine.version,device:engine.device,recording:engine.recording,metrics:engine.metrics,diagnostics:engine.diagnostics}:null,positional:{thirds:pos?.thirds||[],physicalThirds:pos?.physicalThirds||[],sides:pos?.sides||[],sprintProfile,sprintPoints:sprintProfile.points||[],role,attackDirection,coordinateModel:'physical_goal_a_u0_goal_b_u1_v1',fieldCalibrated:!!pos?.fieldCalibrated,pitchId:selectedPitch?.id??null,pitchName:selectedPitch?.name||null,goalALabel:selectedPitch?.goal_a_label||'Portería A',goalBLabel:selectedPitch?.goal_b_label||'Portería B',pitchDistanceFromTrackM:selectedPitch?.distanceFromTrackM??null,autoBounds:pos?.autoBounds||null,lengthM:pos?.lengthM??null,widthM:pos?.widthM??null},speed:{zones:speedZones(samples),sprintModel:relative.model,relativeSprintCutoffKmh:relative.cutoffKmh,robustTopKmh:relative.robustTopKmh,runningFloorKmh:relative.runningFloorKmh,candidatePeakCount:relative.candidatePeakCount,sprints:relative.sprints,absoluteSprintReferenceKmh:ABSOLUTE_SPRINT_REFERENCE_KMH,absoluteSprintCount18:absoluteSprints18.length,highIntensityThresholdKmh:highIntensityKmh,highIntensityRunCount:hiRuns.length,highIntensityRuns:hiRuns.slice(0,80),rawTopSpeedKmh:+rawTopSpeedKmh.toFixed(2),smoothedTopSpeedKmh:+smoothedTopSpeedKmh.toFixed(2),fitTopSpeedKmh:fitTopSpeedKmh==null?null:+fitTopSpeedKmh.toFixed(2),peakSpeed3sKmh:Number.isFinite(Number(engine?.metrics?.maxSpeed3sMps))?+(Number(engine.metrics.maxSpeed3sMps)*3.6).toFixed(2):null,peakSpeed5sKmh:Number.isFinite(Number(engine?.metrics?.maxSpeed5sMps))?+(Number(engine.metrics.maxSpeed5sMps)*3.6).toFixed(2):null,topSpeedMethod,topSpeedEvent:topSpeedSample?{tSec:+Number(topSpeedSample.tSec).toFixed(1),speedKmh:+Number(topSpeedKmh).toFixed(2)}:null,busiestBlock:busiestDistanceBlock(samples,durationS),accelerations:accels.accelerations,decelerations:accels.decelerations,accelerationThresholdMs2:accels.accelThresholdMs2,decelerationThresholdMs2:accels.decelThresholdMs2,accelerationModel:accels.model,speedSeries:downsampleSeries(samples,'speedKmh',360,true)},heartRate:{referenceMaxBpm:refMax,zones:hrZones(samples,refMax),series:downsampleSeries(samples,'hr'),recoveryEvents:hrRecovery.events,recoverySummary:hrRecovery.summary},workload:{distancePerMin:movingTimeS?+(distanceM/(movingTimeS/60)).toFixed(1):0,highIntensityDistancePerMin:movingTimeS?+(highIntensityDistanceM/(movingTimeS/60)).toFixed(1):0,avgCadence:cadences.length?+mean(cadences).toFixed(1):null}};
...
export function toSupabaseRow(matchId,playerId,a){const p=a?.analysisDetail?.positional||{};return{match_id:matchId,player_id:playerId,source_format:a.sourceFormat||'fit',source_started_at:a.sourceStartedAt,duration_s:a.durationS,moving_time_s:a.movingTimeS,distance_m:a.distanceM,top_speed_kmh:a.topSpeedKmh,avg_speed_kmh:a.avgSpeedKmh,sprint_count:a.sprintCount,high_intensity_distance_m:a.highIntensityDistanceM,avg_hr:a.avgHr,max_hr:a.maxHr,sample_count:a.sampleCount,has_gps:a.hasGps,has_hr:a.hasHr,heatmap_grid:a.heatmapGrid,zone_grid:a.zoneGrid,avg_position:a.avgPosition,trail:a.trail,speed_zones:a.speedZones,analysis_detail:a.analysisDetail||{},pitch_id:p.pitchId??null,attack_direction:p.attackDirection===-1?-1:1,analysis_version:7,updated_at:new Date().toISOString()}}
export function fromSupabaseRow(row){if(!row)return null;const a={sourceFormat:row.source_format||'fit',sourceStartedAt:row.source_started_at,durationS:Number(row.duration_s)||0,movingTimeS:Number(row.moving_time_s)||0,distanceM:Number(row.distance_m)||0,topSpeedKmh:Number(row.top_speed_kmh)||0,avgSpeedKmh:Number(row.avg_speed_kmh)||0,sprintCount:Number(row.sprint_count)||0,highIntensityDistanceM:Number(row.high_intensity_distance_m)||0,avgHr:row.avg_hr==null?null:Number(row.avg_hr),maxHr:row.max_hr==null?null:Number(row.max_hr),sampleCount:Number(row.sample_count)||0,hasGps:!!row.has_gps,hasHr:!!row.has_hr,heatmapGrid:row.heatmap_grid||[],zoneGrid:row.zone_grid||[],avgPosition:row.avg_position||null,trail:row.trail||[],speedZones:row.speed_zones||[],analysisDetail:row.analysis_detail||{}};a.analysisDetail.positional=a.analysisDetail.positional||{};a.analysisDetail.positional.attackDirection=row.attack_direction===-1?-1:(a.analysisDetail.positional.attackDirection===-1?-1:1);const speed=a.analysisDetail.speed||(a.analysisDetail.speed={}),fitPeak=a.analysisDetail.gpsEngine?.metrics?.rawFitMaxSpeedKmh!=null?Number(a.analysisDetail.gpsEngine.metrics.rawFitMaxSpeedKmh):null;if(Number.isFinite(fitPeak)){a.topSpeedKmh=fitPeak;if(speed.topSpeedMethod!=='fit-peak')speed.topSpeedEvent=null;speed.fitTopSpeedKmh=fitPeak;speed.topSpeedMethod='fit-peak'}return a}
function physicalThirdsFromZoneGrid(zoneGrid){
  if(!Array.isArray(zoneGrid)||!zoneGrid.length)return null;
  const cols=Array(6).fill(0);
  for(const row of zoneGrid)for(let i=0;i<Math.min(6,row?.length||0);i++)cols[i]+=Number(row[i])||0;
  const total=cols.reduce((a,b)=>a+b,0);if(!(total>0))return null;
  return[(cols[0]+cols[1])/total,(cols[2]+cols[3])/total,(cols[4]+cols[5])/total];
}
function sprintThirdsForDirection(points,direction){
  const thirds=[0,0,0],valid=(points||[]).filter(x=>Number.isFinite(Number(x?.u)));
  for(const x of valid){const physicalU=clamp(Number(x.u),0,.9999),u=direction<0?1-physicalU:physicalU;thirds[Math.min(2,Math.floor(u*3))]++}
  const n=valid.length||1;return thirds.map(v=>v/n);
}
export function setAttackDirection(analysis,direction){
  if(!analysis)return analysis;
  const target=Number(direction)===-1?-1:1;
  const clone=typeof structuredClone==='function'?structuredClone(analysis):JSON.parse(JSON.stringify(analysis));
  clone.analysisDetail=clone.analysisDetail||{};clone.analysisDetail.positional=clone.analysisDetail.positional||{};
  const p=clone.analysisDetail.positional,speed=clone.analysisDetail.speed||{};

  // v222: JAMÁS se tocan heatmapGrid, zoneGrid, avgPosition, trail ni coordenadas de sprint.
  // Esos datos representan la posición física real Huerta (u=0) -> Muro (u=1).
  let physicalThirds=Array.isArray(p.physicalThirds)&&p.physicalThirds.length===3?p.physicalThirds.slice():physicalThirdsFromZoneGrid(clone.zoneGrid);
  if(!physicalThirds){const current=Array.isArray(p.thirds)?p.thirds.slice():[0,0,0],oldDir=Number(p.attackDirection)===-1?-1:1;physicalThirds=oldDir<0?current.reverse():current}
  p.physicalThirds=physicalThirds.slice();
  p.thirds=target<0?physicalThirds.slice().reverse():physicalThirds.slice();

  const sprintPoints=Array.isArray(p.sprintPoints)?p.sprintPoints:(Array.isArray(p.sprintProfile?.points)?p.sprintProfile.points:[]);
  const sprintThirds=sprintThirdsForDirection(sprintPoints,target);
  p.sprintProfile={...(p.sprintProfile||{}),points:sprintPoints,thirds:sprintThirds};
  p.sprintPoints=sprintPoints;
  p.attackDirection=target;
  p.coordinateModel='physical_goal_a_u0_goal_b_u1_v1';

  const oldRole=p.role||{},physicalAvg=clone.avgPosition||{u:.5,v:.5},tacticalAvg={...physicalAvg,u:target<0?1-Number(physicalAvg.u||0):Number(physicalAvg.u||0)};
  p.role=roleFromFeatures({thirds:p.thirds,sides:p.sides||[],spreadU:Number(oldRole.spreadU)||0,spreadV:Number(oldRole.spreadV)||0,sprintCount:Number(clone.sprintCount)||speed.sprints?.length||0,durationS:Number(clone.durationS)||0,avgPosition:tacticalAvg,attackDirection:target,sprintThirds,sprintSides:p.sprintProfile?.sides||[]});
  return clone;
}
