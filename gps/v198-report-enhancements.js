...
const state={minute:60,playing:false,raf:0,lastTs:0,durationSec:18,slider:null,play:null,label:null,trailCanvas:null,terrainMap:null,terrainLine:null,terrainDot:null,terrainRoute:[],pitchId:null};
...
function drawCurrentTrail(){
  const a=analysis();
  if(!a||!state.trailCanvas)return;
  drawMovementTrailAtMinute(state.trailCanvas,a,state.minute);
}
...
async function mountTerrain(){
  ...
  state.terrainLine=L.polyline(latlngs,{
    color:'#ffd24a',weight:3,opacity:.92,lineCap:'round',lineJoin:'round'
  }).addTo(map);

  state.terrainDot=L.circleMarker(latlngs.at(-1),{
    radius:4.5,color:'#111',weight:1.5,fillColor:'#ffd24a',fillOpacity:1
  }).addTo(map);

  const cornerLatLngs=(pitch.corners||[]).map(cornerPoint).filter(Boolean).map(c=>[c.lat,c.lng]);
