...
function closeOverlay(){document.querySelector('[data-patx-gps-report-overlay]')?.remove();document.body.classList.remove('patx-v204-report-open')}
function openReport(matchId,playerId){if(!matchId||!playerId)return;ensureOverlay().querySelector('iframe').src=`gps-report.html?match=${encodeURIComponent(matchId)}&player=${encodeURIComponent(playerId)}`}
