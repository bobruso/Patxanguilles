...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==="save"){
      ...
      const {error:saveError}=await supabase.from("players").update({card_url:generation.image_url}).eq("id",playerId);
      if(saveError)throw new Error(`No se pudo guardar la carta: ${saveError.message}`);
      return json({ok:true,message:"Carta guardada en el perfil",card_url:generation.image_url,generation_id:generation.id});
    }
    ...
    const openaiForm=new FormData();openaiForm.append("model","gpt-image-2");openaiForm.append("prompt",prompt);openaiForm.append("size","1024x1536");openaiForm.append("quality","medium");openaiForm.append("output_format","png");
    ...
    const bytes=decodeBase64(b64),path=`generated/${playerId}/card-${generationId}.png`;const {error:uploadError}=await supabase.storage.from("player-photos").upload(path,bytes,{contentType:"image/png",upsert:true});if(uploadError)throw new Error(`No se pudo guardar la carta: ${uploadError.message}`);
    const {data:publicUrlData}=supabase.storage.from("player-photos").getPublicUrl(path),cardUrl=publicUrlData.publicUrl;
    const {error:completeError}=await supabase.from("card_generations").update({status:"completed",image_url:cardUrl,error_message:null,completed_at:new Date().toISOString()}).eq("id",generationId);if(completeError)throw new Error(`No se pudo cerrar la generación: ${completeError.message}`);
