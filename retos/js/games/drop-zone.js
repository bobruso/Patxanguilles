(() => {
  ...
  window.PatxGameRegistry.register('drop-zone', ({container,config={},seed=1,onFinish}) => {
    ...
    let state=Number(seed)||1,level=0,startedAt=0,finished=false,raf=0,last=0,goalX=.5,dir=1,shooting=false,shotT=0,shotGoalX=.5;
    ...
    function reset(){shooting=false;shotT=0;goalX=.2+rnd()*.6;dir=rnd()>.5?1:-1;levelEl.textContent=`Nivel ${level+1} / ${maxLevel}`;statusEl.textContent=`VELOCIDAD ×${(speed()/.34).toFixed(2)}`;helpEl.textContent='Toca para chutar cuando la portería cruce el centro.';}
    ...
    function draw(){
      ...
      const gx=(shooting?shotGoalX:goalX)*560,gw=goalWidth()*560;drawGoal(gx,gw);
    ...
    function resolveShot(){
      const ok=Math.abs(shotGoalX-.5)<goalWidth()*.43;
    ...
    function loop(now){
      ...
      if(!shooting){goalX+=dir*speed()*dt;if(goalX>1-goalWidth()/2){goalX=1-goalWidth()/2;dir=-1;}if(goalX<goalWidth()/2){goalX=goalWidth()/2;dir=1;}}
      else{shotT=Math.min(1,shotT+dt*2.6);if(shotT>=1){shooting=false;resolveShot();}}
      draw();raf=requestAnimationFrame(loop);
    }
    function shoot(e){e?.preventDefault();if(finished||shooting)return;shooting=true;shotT=0;shotGoalX=goalX;statusEl.textContent='¡CHUT!';helpEl.textContent='…';}
