  ...
  window.PatxGameRegistry.register('rhythm-tap', ({container,config={},onFinish}) => {
    ...
    container.innerHTML=`
      <div class="game-surface rhythm-surface">
        ...
        <button class="rhythm-pad" type="button" data-pad aria-label="Tocar al ritmo"><span class="rhythm-target-ring"></span><span data-pulse></span><span class="rhythm-center-dot"></span></button>
    ...
    function advance(now){
      if(finished)return;
      const elapsed=now-baseAt,beat=Math.floor(elapsed/interval);
      if(beat>=beats){if(!tappedForBeat&&currentBeat<beats)errorSum+=1000;return finish();}
      if(beat!==currentBeat){if(currentBeat>=0&&!tappedForBeat)errorSum+=1000;currentBeat=beat;tappedForBeat=false;roundEl.textContent=`Pulso ${beat+1} / ${beats}`;gradeEl.textContent='ESPERA EL CENTRO';gradeEl.className='rhythm-caption';}
    ...
    function tap(){
      if(finished||tappedForBeat)return;
      const now=performance.now(),ideal=baseAt+currentBeat*interval+interval/2,err=Math.abs(now-ideal);errorSum+=Math.min(err,1000);tappedForBeat=true;taps++;scoreEl.textContent=`${taps} / ${beats}`;
    ...
    return{start(){startedAt=performance.now();baseAt=startedAt+380;currentBeat=-1;timeout=setTimeout(()=>{raf=requestAnimationFrame(advance);},320);},destroy(){finished=true;cancelAnimationFrame(raf);clearTimeout(timeout);pad.removeEventListener('pointerdown',tap);}};
  });
})();